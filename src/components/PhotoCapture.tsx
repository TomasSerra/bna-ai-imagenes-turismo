...
export function PhotoCapture({ onCapture, hasPhoto, previewUrl, onReset }: PhotoCaptureProps) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const columnStyles = columnRef.current ? getComputedStyle(columnRef.current) : null;
      const gap = Number.parseFloat(columnStyles?.rowGap ?? '') || 0;
      const availablePhotoHeight = Math.max(0, height - buttonHeight - gap);

      // La tipografía raíz escala con la altura del viewport, por lo que las
      // medidas Tailwind del botón y del gap no son siempre 64 px y 12 px.
      // Medirlas evita que la cámara invada el espacio entre bloques en Full HD.
      setColumnWidth(Math.min(availablePhotoHeight * PHOTO_ASPECT, width));
    };
    update();
  ...
  if (hasPhoto && previewUrl) {
    return (
      <div ref={containerRef} className="flex h-full items-center justify-center">
        <div
          ref={columnRef}
          className="flex max-w-full flex-col gap-[clamp(0.875rem,1.35vh,1.5rem)]"
          style={{ width: columnWidth }}
        >
          <div className="aspect-[3/4] w-full overflow-hidden rounded-2xl border-4 border-white/90 bg-muted shadow-2xl ring-4 ring-[#36c8e8]/35">
          ...
          <Button
            ...
            variant="outline"
            onClick={onReset}
            className="h-16 w-full min-w-max whitespace-nowrap rounded-full border-2 border-white bg-white/95 text-[#003b70] text-2xl hover:bg-white hover:text-[#003b70] [&_svg]:size-7"
          >
  ...
  return (
    <div ref={containerRef} className="flex h-full items-center justify-center">
      <div
        ...
        style={{ width: columnWidth }}
      >
        <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl border-4 border-white/90 bg-muted shadow-2xl ring-4 ring-[#36c8e8]/35">
          {cameraError ? (
        ...
        <Button
          ...
          className="h-16 w-full rounded-full border-2 border-white bg-gradient-to-r from-[#003b70] via-[#006da8] to-[#29b9dd] text-2xl font-kievit-black text-white shadow-lg hover:brightness-110 [&_svg]:size-7"
        >
